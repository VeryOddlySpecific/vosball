"""vosball.engine.core — the VOS scoring computation, lifted from run_vos.py.

Deterministic transforms over a parsed player row (a dict of CSV cells) and a
weights/config dict: park adjustment, mode/decay resolution, the v6 logistic
Reach model, per-tool hitter/pitcher scoring, the development/readiness/age/
personality/draft adjustment stack, WAR projection, and the build_*_row
integrators. No file or network I/O happens here. Output is byte-identical to
the pre-refactor run_vos.py (guarded by tests/test_golden.py).

Lifted verbatim in the Phase 1 extraction; can be split into finer submodules
(scoring/adjustments/park/assembly) in a follow-up.
"""
...
from vosball.engine.constants import (
    BASERUNNING_STEAL_COLS, CTRL_COL_ALTERNATIVES, POT_PITCH_COLUMN_TO_TYPE,
    PITCH_SPEED_TIERS, PITCH_BREAK_PLANES, PERSONALITY_CSV_TO_CONFIG,
    PRONE_CATEGORY_TO_NUMERIC, HITTER_POSITIONS, LEVEL_LABEL_TO_CONFIG,
)
...
from vosball.engine.tiers import classify_vos_tier, tier_for_player_role, _resolve_tier_bands

logger = logging.getLogger(__name__)

__all__ = ['_blend_alpha', '_build_single_park_config', '_ceiling_tier_band', '_classify_ceiling_tier', '_decay_cfg_for_mode', '_decay_tool_for_key', '_development_dampener', '_has_ceiling', '_has_v6_reach', '_interp1d', '_is_single_park_format', '_mode_block', '_personality_bucket_from_cell', '_resolve_with_decay', '_v6_extract_features', '_v6_reach_score', '_weighted_sum_from_dict', 'age_adjustment', 'apply_park_adjustments', 'build_hitter_row', 'build_pitcher_row', 'development_adjustment', 'draft_age_modifier', 'draft_role_penalty', 'get_league_key_for_config', 'get_league_label', 'get_player_park_config', 'get_team_display', 'hitter_baserunning_score', 'hitter_batting_score', 'hitter_defense_score', 'hitter_position_scores', 'is_pitcher', 'personality_adjustment', 'pitcher_ability_score', 'pitcher_arsenal_score', 'pitcher_combined_score', 'project_archetype_war', 'readiness_adjustment_hitter', 'readiness_adjustment_pitcher']
